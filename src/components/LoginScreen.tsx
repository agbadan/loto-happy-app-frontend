...
interface Ball { id: number; size: number; x: number; y: number; duration: number; delay: number; number: number; color: string; }
const BALL_COLORS = ["#FFD700", "#FF1744", "#2196F3", "#4CAF50", "#FF6B00", "#9C27B0", "#00BCD4", "#E91E63", "#FF9800", "#673AB7"];
...
const generateBalls = (): Ball[] => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        let number: number;
        do { number = Math.floor(Math.random() * 90) + 1; } while (usedNumbers.has(number) && usedNumbers.size < 90);
        usedNumbers.add(number);
        const color = BALL_COLORS[Math.floor(Math.random() * BALL_COLORS.length)];
        balls.push({ id: i, size, color, number, x: Math.random() * 110 - 5, y: Math.random() * 110 - 5, duration: Math.random() * 15 + 25, delay: Math.random() * 8 });
    }
    return balls;
};

export function LoginScreen({ onNavigateToPassword, onNavigateToRegistration }: LoginScreenProps) {
  const { error: authError } = useAuth();
  const [countryCode, setCountryCode] = useState("+228");
  const [phoneNumber, setPhoneNumber] = useState("");
  const [balls] = useState<Ball[]>(generateBalls());
  
  // Le nom de l'état est bien 'infoModalOpen'
  ...
  const handleContinue = () => {
    if (!phoneNumber) { toast.error("Veuillez entrer un email ou un numéro de téléphone."); return; }
    const isEmail = phoneNumber.includes('@');
    if (isEmail) { onNavigateToPassword(phoneNumber); return; }
    const validation = validatePhoneNumber(phoneNumber, countryCode);
    if (!validation.isValid) { toast.error(validation.message || "Numéro de téléphone invalide"); return; }
    const fullNumber = `${countryCode}${phoneNumber}`;
    onNavigateToPassword(fullNumber);
  };

  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-12" style={{ backgroundColor: isDark ? '#121212' : '#F4F4F7' }}>
      <div className="absolute inset-0 overflow-hidden">
        {balls.map((ball) => {
            ...
            return (
                <motion.div key={ball.id} className="absolute flex items-center justify-center rounded-full" style={{ width: ball.size, height: ball.size, left: `${ball.x}%`, top: `${ball.y}%`, willChange: "transform", transform: "translate3d(0, 0, 0)", background: `radial-gradient(circle at 28% 25%, ${ball.color}FF 0%, ${ball.color}A0 100%)`, boxShadow: `0 30px 70px rgba(0,0,0,0.12), inset -10px -10px 25px rgba(0,0,0,0.12), inset 8px 8px 20px rgba(255,255,255,0.2)` }} animate={chaoticAnimation} transition={transitionConfig}>
                    ...
                    <div className="absolute flex items-center justify-center rounded-full bg-white" style={{ width: circleSize, height: circleSize, boxShadow: `0 0 0 ${ball.size * 0.04}px ${ball.color}, inset 0 3px 8px rgba(0,0,0,0.1)` }}>
                        <span className="font-bold" style={{ fontSize: `${fontSize}px`, color: "#1C1C1E", textShadow: "0 1px 2px rgba(0,0,0,0.1)", fontFamily: "Inter, system-ui, sans-serif" }}>{ball.number}</span>
                    </div>
                </motion.div>
            );
        })}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="relative z-10 w-full max-w-md">
        <div className="rounded-3xl px-8 py-16 shadow-2xl" style={{ backgroundColor: isDark ? 'rgba(28, 28, 30, 0.75)' : 'rgba(255, 255, 255, 0.7)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: isDark ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid rgba(255, 255, 255, 0.5)' }}>
          ...
          <motion.div className="space-y-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.6 }}>
              ...
              <div className="flex gap-2">
                {!phoneNumber.includes('@') && (
                  <Select value={countryCode} onValueChange={setCountryCode}>
                    <SelectTrigger className="w-32 focus:ring-2 focus:ring-[#FFD700]/20" style={{ backgroundColor: isDark ? '#1C1C1E' : '#FFFFFF', borderColor: isDark ? 'rgba(255, 255, 255, 0.1)' : '#d1d1d6' }}><SelectValue /></SelectTrigger>
                    <SelectContent>{COUNTRIES.map((country) => (<SelectItem key={country.code} value={country.code}><span className="flex items-center gap-2"><span>{country.flag}</span><span>{country.code}</span></span></SelectItem>))}</SelectContent>
                  </Select>
                )}
            ...
            <p className="text-center text-sm" style={{ color: isDark ? '#8E8E93' : '#6e6e73' }}>
              ...
              <button onClick={() => onNavigateToRegistration(phoneNumber)} className="font-semibold text-[#FFD700] hover:underline focus:outline-none">Inscrivez-vous</button>
            </p>
            ...
            <Button variant="outline" className="w-full border-2 hover:border-[#FFD700] hover:bg-[#FFD700]/5 transition-all" style={{ backgroundColor: isDark ? '#1C1C1E' : '#FFFFFF', borderColor: isDark ? 'rgba(255, 255, 255, 0.1)' : '#d1d1d6', color: isDark ? '#EAEAEA' : '#1C1C1E' }} onClick={() => setInfoModalOpen(true)}>
      ...
      <Dialog open={infoModalOpen} onOpenChange={setInfoModalOpen}>
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    La connexion avec Google sera bientôt disponible pour une expérience encore plus rapide et sécurisée.
                </DialogDescription>
            </DialogHeader>
            <Button onClick={() => setInfoModalOpen(false)} className="mt-4 w-full bg-[#FFD700] text-[#121212] hover:bg-[#FFD700]/90">
                J'ai compris
            </Button>
        </DialogContent>
      </Dialog>
